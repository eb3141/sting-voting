...
# File paths (use absolute paths to ensure consistent file locations)
script_dir = os.path.dirname(os.path.abspath(__file__))
votes_file = os.path.join(script_dir, "votes.csv")
excel_file = os.path.join(script_dir, "fOutputAndaReport.xlsx")
export_file = os.path.join(script_dir, "Voting_Results_Summary.xlsx")
...
def save_vote(judge_name, applicant_name, status, rating, comment, original_status, original_rating):
    try:
        ...
        if os.path.exists(votes_file):
            ...
            st.error(f"❌ Error: Votes file not found at {votes_file}")
    except Exception as e:
        st.error(f"❌ Error saving vote: {str(e)}\nFile path: {votes_file}")
